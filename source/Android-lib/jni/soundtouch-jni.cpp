...
static void _processFile(SoundTouch *pSoundTouch, const char *inFileName, const char *outFileName)
{
    ...
    int nChannels;
    int buffSizeSamples;
    SAMPLETYPE sampleBuffer[BUFF_SIZE];    // open input file
    soundstretch::WavInFile inFile(inFileName);
    int sampleRate = inFile.getSampleRate();
    int bits = inFile.getNumBits();
    nChannels = inFile.getNumChannels();

    // create output file
    soundstretch::WavOutFile outFile(outFileName, sampleRate, bits, nChannels);
...
extern "C" DLL_PUBLIC void Java_net_surina_soundtouch_SoundTouch_setChannels(JNIEnv *env, jobject thiz, jlong handle, jint numChannels)
{
    SoundTouch *ptr = (SoundTouch*)handle;
    ptr->setChannels(numChannels);
}
